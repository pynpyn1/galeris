    ...
    <section class="section">
        <div class="card">
            <div class="card-header">
                <p class="mb-0">Edit Template ChatBot</p>
            </div>
            <div class="card-body">
                ...
                <form action="{{ route('manage.chatbot.update', $chatbot->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="user_id" id="user_id" class="form-select" required>
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ $chatbot->user_id == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
